...
> `webtyp/webtyp/input` (decorados con UI); ver §8 «Validation design».
...
`webtyp/model` is the input-validation boundary for the ecosystem:
...
los kinds (`webtyp/webtyp/input`, kinds custom en su propio paquete) — esos siempre compilan porque
...
- **widgets**: cualquier `model.Widget`. `webtyp/webtyp/input` (`input.Text()`, `input.Email()`, …)
